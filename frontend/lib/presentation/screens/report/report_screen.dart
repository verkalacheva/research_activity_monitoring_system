...
import 'package:research_activity_monitoring_system/presentation/features/report/report_constants.dart';
import 'package:research_activity_monitoring_system/presentation/features/report/widgets/report_dashboard_chart_shell.dart';
...
class _ReportScreenState extends State<ReportScreen> {
  ...
  void _openReportDetail(String reportId) {
    ...
    final reportFilterIds = ReportUiConstants.reportFilters[reportId] ?? [];
    final filters = reportFilterIds.map((id) {
      final meta = context.strings.reportFilterMetadata[id] ?? {};
      return {
        'field': id,
        'operator': 'eq',
        'value': '',
    ...
    setState(() {
      ...
      final availableSorts = ReportUiConstants.reportSorts[reportId] ?? ['id'];
          ...
          return Dialog.fullscreen(
            child: Scaffold(
              ...
              appBar: AppBar(
                ...
                title: Text(
                  context.strings.reportTitles[reportId] ?? context.strings.reportDefaultTitle,
  ...
  Future<void> _handlePrint() async {
    ...
    try {
      ...
      if (mounted) {
        _printNative(
          result,
          context.strings.reportTitles[_selectedReportId] ?? context.strings.reportDefaultTitle,
        );
      }
    } catch (e) {
      if (mounted) {
  ...
  void _printNative(Map<String, dynamic> reportData, String title) {
          ...
          child: Column(
            ...
            children: [
              Padding(
                ...
                child: Text(context.strings.tr('home.nav.reports'), style: AppTextStyles.h2),
              ),
                          ...
                          ...((_selectors?['report_types'] as List?) ?? []).map((report) {
                            ...
                            return Container(
                              ...
                              child: ListTile(
                                ...
                                title: Text(
                                  context.strings.reportTitles[reportId] ?? reportId.toString(),
                                  style: TextStyle(
  ...
  Widget _buildDashboardChart(String title, IconData icon, Widget chart) {
    return ReportDashboardChartShell(
      title: title,
      icon: icon,
      chart: chart,
  ...
  void _addFilter() {
    if (_selectedReportId == null) return;
    final reportFilterIds = ReportUiConstants.reportFilters[_selectedReportId] ?? [];
    if (reportFilterIds.isNotEmpty) {
      final filterId = reportFilterIds[0];
      final meta = context.strings.reportFilterMetadata[filterId] ?? {};
      _activeFilters.add({
        'field': filterId,
        'operator': 'eq',
        'value': '',
