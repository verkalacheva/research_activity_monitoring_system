...
import 'package:research_activity_monitoring_system/presentation/screens/researcher/researcher_form_screen.dart';
...
class _ResearcherListScreenState extends State<ResearcherListScreen> {
  ...
  void _onSavedAfterGlobalSync() {
    _refreshList();
    final sid = _selectedResearcher?.id;
    if (sid != null) {
      _refreshSelectedResearcher(sid);
    }
  }
  ...
  void _syncAllEmployees() {
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text(
            'Запущены три этапа: каталоги ORCID/OpenAlex, GitHub, затем поиск в интернете по каждому сотруднику',
          ),
        ),
      );
    }
